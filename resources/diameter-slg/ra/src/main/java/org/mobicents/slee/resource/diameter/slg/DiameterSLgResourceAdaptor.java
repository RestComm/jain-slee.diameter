...
/*
  public void sessionCreated(ServerSLgSession session) {
    SLgMessageFactoryImpl sessionMsgFactory = new SLgMessageFactoryImpl(session.getSessions().get(0), stack, new DiameterIdentity[]{});

    // Set the first configured Application-Id as default for message factory
    ApplicationId firstAppId = authApplicationIds.get(0);
    sessionMsgFactory.setApplicationId(firstAppId.getVendorId(), firstAppId.getAuthAppId());

    SLgServerSessionImpl serverActivity = new SLgServerSessionImpl(sessionMsgFactory, slgAvpFactory, session, this, null, null, stack);
    //session.addStateChangeNotification(serverActivity);
    //addActivity(serverActivity);
    serverActivity.setSessionListener(this);
  }

  public void sessionCreated(ClientSLgSession session) {
    SLgMessageFactoryImpl sessionMsgFactory = new SLgMessageFactoryImpl(session.getSessions().get(0), stack, new DiameterIdentity[]{});
    ...
    SLgClientSessionImpl clientActivity = new SLgClientSessionImpl(sessionMsgFactory, slgAvpFactory, session, this, null, null, stack);
    //session.addStateChangeNotification(serverActivity);
    //addActivity(serverActivity);
...
/*
    private DiameterActivity createSLgServerSessionActivity(Request request) throws CreateActivityException {
      ...
      SLgServerSessionImpl activity = new SLgServerSessionImpl(ra.slgMessageFactory, ra.slgAvpFactory, session, (EventListener<Request, Answer>) session, (DiameterIdentity) null, (DiameterIdentity) null, stack);
...
/*
    private DiameterActivity createSLgClientSessionActivity(Request request) throws CreateActivityException {
      ...
      SLgClientSessionImpl activity = new SLgClientSessionImpl(ra.slgMessageFactory, ra.slgAvpFactory, session, (EventListener<Request, Answer>) session, (DiameterIdentity) null, (DiameterIdentity) null, stack);
      addActivity(activity, false);
...
/*
    public SLgServerSessionActivity createSLgServerSessionActivity(DiameterIdentity destinationHost, DiameterIdentity destinationRealm) throws CreateActivityException {
      try {
        ...
        SLgServerSessionImpl activity = new SLgServerSessionImpl(ra.slgMessageFactory, ra.slgAvpFactory, session, (EventListener<Request, Answer>) session, destinationHost, destinationRealm, stack);
        addActivity(activity, false);
        return activity;
      }
      catch (Exception e) {
    ...
    public SLgClientSessionActivity createSLgClientSessionActivity(DiameterIdentity destinationHost, DiameterIdentity destinationRealm) throws CreateActivityException {
      try {
        ...
        SLgClientSessionImpl activity = new SLgClientSessionImpl(ra.slgMessageFactory, ra.slgAvpFactory, session, (EventListener<Request, Answer>) session, destinationHost, destinationRealm, stack);
        addActivity(activity, false);
        return activity;
      }
      catch (Exception e) {
