...
//        return isRequest ? new LCSRoutingInfoRequestImpl(message) : new ProvideLocationAnswerImpl(message);
...
/*
  public void sessionCreated(ServerSLhSession session) {
    SLhMessageFactoryImpl sessionMsgFactory = new SLhMessageFactoryImpl(session.getSessions().get(0), stack, new DiameterIdentity[]{});

    // Set the first configured Application-Id as default for message factory
    ApplicationId firstAppId = authApplicationIds.get(0);
    sessionMsgFactory.setApplicationId(firstAppId.getVendorId(), firstAppId.getAuthAppId());

    SLhServerSessionImpl serverActivity = new SLhServerSessionImpl(sessionMsgFactory, slhAvpFactory, session, this, null, null, stack);
    //session.addStateChangeNotification(serverActivity);
    //addActivity(serverActivity);
    serverActivity.setSessionListener(this);
  }

  public void sessionCreated(ClientSLhSession session) {
    SLhMessageFactoryImpl sessionMsgFactory = new SLhMessageFactoryImpl(session.getSessions().get(0), stack, new DiameterIdentity[]{});
    ...
    SLhClientSessionImpl clientActivity = new SLhClientSessionImpl(sessionMsgFactory, slhAvpFactory, session, this, null, null, stack);
    //session.addStateChangeNotification(serverActivity);
    //addActivity(serverActivity);
      ...
      if (activity == null) {
        if (message.isRequest()) {
          switch(message.getCommandCode()) {
            ...
            case LCSRoutingInfoRequest.COMMAND_CODE:
              return createSLhClientSessionActivity((Request) message);
          }
        }
        else {
...
/*
    private DiameterActivity createSLhServerSessionActivity(Request request) throws CreateActivityException {
      ...
      SLhServerSessionImpl activity = new SLhServerSessionImpl(ra.slhMessageFactory, ra.slhAvpFactory, session, (EventListener<Request, Answer>) session, (DiameterIdentity) null, (DiameterIdentity) null, stack);
...
/*
    private DiameterActivity createSLhClientSessionActivity(Request request) throws CreateActivityException {
      ...
      SLhClientSessionImpl activity = new SLhClientSessionImpl(ra.slhMessageFactory, ra.slhAvpFactory, session, (EventListener<Request, Answer>) session, (DiameterIdentity) null, (DiameterIdentity) null, stack);
      addActivity(activity, false);
...
/*
    public SLhServerSessionActivity createSLhServerSessionActivity(DiameterIdentity destinationHost, DiameterIdentity destinationRealm) throws CreateActivityException {
      try {
        ...
        SLhServerSessionImpl activity = new SLhServerSessionImpl(ra.slhMessageFactory, ra.slhAvpFactory, session, (EventListener<Request, Answer>) session, destinationHost, destinationRealm, stack);
        addActivity(activity, false);
        return activity;
      }
      catch (Exception e) {
    ...
    public SLhClientSessionActivity createSLhClientSessionActivity(DiameterIdentity destinationHost, DiameterIdentity destinationRealm) throws CreateActivityException {
      try {
        ...
        SLhClientSessionImpl activity = new SLhClientSessionImpl(ra.slhMessageFactory, ra.slhAvpFactory, session, (EventListener<Request, Answer>) session, destinationHost, destinationRealm, stack);
        addActivity(activity, false);
        return activity;
      }
      catch (Exception e) {
